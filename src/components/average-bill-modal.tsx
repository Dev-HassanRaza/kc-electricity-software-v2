...
export default function AverageBillModal({ onClose }: { onClose: () => void }) {
  ...
  const [rdate, setRdate] = useState('') // Reading Date
  const [isdate, setIsdate] = useState('') // Issue Date
  const [duedate, setDuedate] = useState('') // Due Date

  const [previousReading, setPreviousReading] = useState<number>(0)
  const [averageUnit, setAverageUnit] = useState<number>(0)
  ...
  useEffect(() => {
    const fetchHistoryForParty = async () => {
      ...
      try {
        ...
        if (res.ok && data.data) {
          // Filter history to display all or only average bills?
          // Displaying all bills is helpful, but highlighting or filtering is good.
          // Let's show all bills for this party.
          setBillsHistory(data.data)

          // Prefill previous reading from the latest bill
  ...
  // Calculate present reading dynamically from average unit input
  useEffect(() => {
    setPresentReading(previousReading + averageUnit)
  }, [previousReading, averageUnit])
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (averageUnit <= 0) {
      setError('Please enter average units')
      return
    }
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) {
        setSuccess('Average bill issued successfully')
        // Refresh history
        ...
        setError(data.error || 'Failed to save average bill')
      }
    } catch {
      setError('Network error saving average bill')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    if (!confirm('Are you sure you want to delete this average bill? This will also revert the synchronized reading.')) return
      ...
      <div className="w-full max-w-xl overflow-hidden border bg-white border-slate-200 backdrop-blur-md rounded-2xl shadow-2xl shadow-slate-300/40 text-slate-900 flex flex-col max-h-[95vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <form onSubmit={handleSave} className="space-y-3 text-xs">
            ...
            {/* Reading and Average Unit Fields */}
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  type="number"
                  value={averageUnit}
                  onChange={(e) => setAverageUnit(parseFloat(e.target.value) || 0)}
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-wider font-semibold text-slate-500 mb-1">
                  Present Reading (Calc)
                </label>
                <input
                  type="number"
                  value={presentReading}
                  disabled
                  className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-550 font-medium focus:outline-none font-semibold"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-4 pt-2">
              {/* Readings Grid (Table) */}
              <div className="col-span-3 border border-slate-200 bg-slate-50/50 rounded-xl flex flex-col max-h-[220px] overflow-y-auto font-sans">
                <table className="w-full border-collapse text-left text-[11px]">
                  <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 font-bold text-slate-500">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right">Average Unit</th>
                    </tr>
                  </thead>
                    ...
                    {loadingHistory ? (
                      ...
                      billsHistory.map((b) => (
                        <tr
                          ...
                          } ${b.ext?.includes('Average') ? 'text-sky-700 font-semibold' : ''}`}
                        >
                          <td className="px-3 py-1.5 text-slate-500 font-mono">
                            {b.bdate ? new Date(b.bdate).toISOString().split('T')[0] : 'N/A'}
                          </td>
                          <td className="px-3 py-1.5 text-right text-slate-700">{b.reding.toFixed(2)}</td>
